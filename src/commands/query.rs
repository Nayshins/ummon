...
// No global configuration needed - using shared function from llm_integration.rs
...
pub async fn run(
    ...
    // Get LLM configuration using the shared function
    ...
    let response = match process_query(&kg, instruction, &options, &config).await? {

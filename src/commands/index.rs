...
use crate::graph::entity::{
    BaseEntity, EntityId, EntityType, FunctionEntity, ModuleEntity, TypeEntity,
    VariableEntity,
};
...
pub async fn run(path: &str) -> Result<()> {
    ...
    index_entities(path, &mut kg, &mut function_map, &mut type_map, &mut domain_concepts, &mut indexed_files)?;

    // Second pass: Build relationships
    println!("Pass 2: Building relationships...");
    index_relationships(path, &mut kg, &function_map, &type_map, &domain_concepts, &indexed_files)?;
    ...
    for entry in walker {
        ...
        if let Some(mut parser) = get_parser_for_file(path) {
            ...
            let module_entity = ModuleEntity {
                ...
                children: Vec::new(),
                imports: module_info.imports.iter().map(|imp| imp.module_name.clone()).collect(),
            };
            ...
            for func in functions {
                ...
                base.containing_entity = func.containing_type.as_ref().map(|t| EntityId::new(&format!("type::{}", t)));
            ...
            for type_def in types {
                ...
                let type_entity = TypeEntity {
                    base,
                    fields: type_def.fields.iter().map(|f| EntityId::new(&format!("field::{}::{}", key, f.name))).collect(),
                    methods: type_def.methods.iter().map(|m| EntityId::new(&format!("method::{}::{}", key, m))).collect(),
                    supertypes: type_def.super_types.iter().map(|s| EntityId::new(&format!("type::{}", s))).collect(),
        ...
        if let Some(mut parser) = get_parser_for_file(path) {
            ...
            for call in calls {
                ...
                if let Some(callee_key) = &call.fully_qualified_name {
                    ...
                    if let Err(e) = kg.create_relationship(caller_id, callee_id, RelationshipType::Calls) {
                ...
                if type_def.file_path == file_path {
                    ...
                    for super_type in &type_def.super_types {
                        ...
                        let rel_type = match type_def.kind {
                            crate::parser::language_support::TypeKind::Class => RelationshipType::Inherits,
                            crate::parser::language_support::TypeKind::Struct => RelationshipType::Inherits,
                    ...
                    for method in &type_def.methods {
                        ...
                        if let Err(e) = kg.create_relationship(type_id.clone(), method_id, RelationshipType::Contains) {
                            println!("Failed to create contains relationship (method): error: {}", e);
                        }
                    }
                    ...
                    for field in &type_def.fields {
                        let field_id = EntityId::new(&format!("{}::field::{}", key, field.name));
                        if let Err(e) = kg.create_relationship(type_id.clone(), field_id, RelationshipType::Contains) {
                            println!("Failed to create contains relationship (field): error: {}", e);
                        }
                    }
                }
            }
            ...
            for import in &module_info.imports {
                ...
                if let Err(e) = kg.create_relationship(module_id.clone(), imported_module_id, RelationshipType::Imports) {
...
fn infer_domain_model(
    ...
    for (name, concept) in domain_concepts {
        ...
        for entity_ref in &concept.related_entities {
            ...
            if let Err(e) = kg.create_relationship(entity_id.clone(), related_id, RelationshipType::RepresentedBy) {
                println!("Failed to create domain representation relationship: error: {}", e);
            }
        }
    }
    ...
    if use_llm {
        ...
        for entry in walker {
            ...
            let extension = file_path_obj.extension()
                .and_then(|ext| ext.to_str())
                .unwrap_or("");
                
            // Skip non-source files (binaries, images, etc.)
            let is_source_file = matches!(extension, "rs" | "py" | "js" | "ts" | "c" | "cpp" | "h" | "hpp" | "java" | "go" | "rb");
            ...
            if content.len() > 100000 {
                println!("  Skipping very large file: {} ({} bytes)", file_path, content.len());
                continue;
            }
            ...
            for entity in domain_entities {
                println!("  Found domain entity: {} ({})", entity.name, 
                ...
                let attributes: Vec<String> = entity.attributes.keys()
                    .map(|k| k.clone())
                    .collect();
    ...
    let relationships_to_create: Vec<(EntityId, EntityId)> = {
        ...
        for i in 0..domain_entities.len() {
            ...
            let tech_entities1 = kg.get_related_entities(&entity1_id, Some(&RelationshipType::RepresentedBy));
            ...
            for j in (i+1)..domain_entities.len() {
                ...
                let tech_entities2 = kg.get_related_entities(&entity2_id, Some(&RelationshipType::RepresentedBy));
    ...
    for (entity1_id, entity2_id) in relationships_to_create {
        if let Err(e) = kg.create_relationship(entity1_id.clone(), entity2_id.clone(), RelationshipType::RelatesTo) {

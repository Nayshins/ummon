...
use crate::graph::entity::{EntityType, EntityId};
...
impl UmmonRouter {
    ...
    fn find_relevant_files_tool(&self, description: &str, limit: usize) -> Result<Vec<Content>, ToolError> {
        ...
        for entity in search_results.iter() {
            if let Some(path) = entity.path() {
                ...
                let entry = file_relevance.entry(path.to_string()).or_insert((0, Vec::new()));
                entry.0 += 1;
                entry.1.push(format!("- {} ({})", entity.name(), entity.entity_type()));
            }
        }
    ...
    fn explore_relationships_tool(
        ...
        while let Some((current_id, current_depth)) = queue.pop_front() {
            ...
            for rel in current_rels {
                ...
                let source_entity = self.knowledge_graph.get_entity(&crate::graph::entity::EntityId::new(&rel.source_id.to_string()));
                let target_entity = self.knowledge_graph.get_entity(&crate::graph::entity::EntityId::new(&rel.target_id.to_string()));
                
                let source_id_str = rel.source_id.to_string();
                let target_id_str = rel.target_id.to_string();
                let source_name = source_entity.map(|e| e.name()).unwrap_or_else(|| &source_id_str);
                let target_name = target_entity.map(|e| e.name()).unwrap_or_else(|| &target_id_str);
                
                // Store the relationship
                let entry = relationship_map.entry(current_id.clone()).or_insert_with(Vec::new);
                entry.push((rel.relationship_type.to_string(), other_id.clone(), format!("{} → {}", source_name, target_name)));
        ...
        for id in visited.iter() {
            if let Some(e) = self.knowledge_graph.get_entity(&crate::graph::entity::EntityId::new(id)) {
        ...
        result.push_str(&format!(
            ...
            relationship_types.into_iter().collect::<Vec<_>>().join(", ")
        ));
        ...
        for entity in module_entities.iter() {
            if let Some(path) = entity.path() {
                ...
                if parts.len() > 1 {
                    ...
                    module_structure.entry(parent).or_insert_with(Vec::new).push(child);
                }
            }
        }
    ...
    fn generate_medium_architecture(
        ...
        result.push_str("4. For specific tasks, use `find_relevant_files` to identify starting points\n");
    ...
    fn generate_detailed_architecture(
        ...
        if let Some((count, components)) = focused_results {
            ...
            result.push_str(&format!("Found {} components related to this area.\n\n", count));
            ...
            if let Some(focus) = focus_area {
                ...
                if !search_results.is_empty() {
                    ...
                    for entity in search_results.iter().take(10) {
                        if let Ok(rels) = self.knowledge_graph.get_relationships_for_entity(&entity.id().to_string()) {
                            for rel in rels {
                                let rel_string = format!("{} {} {}", rel.source_id, rel.relationship_type, rel.target_id);
                    ...
                    if !relationship_pairs.is_empty() {
                        ...
                        for rel in key_rels {
                            ...
                            let source_name = source_entity.map(|e| format!("{} ({})", e.name(), e.entity_type()))
                                .unwrap_or_else(|| format!("{}", rel.source_id));
                            let target_name = target_entity.map(|e| format!("{} ({})", e.name(), e.entity_type()))
        ...
        fn add_to_tree(tree: &mut HashMap<String, HashMap<String, HashMap<String, Vec<String>>>>, path: &str) {
            let parts: Vec<&str> = path.split('/').collect();
            if parts.len() >= 4 {  // At least 4 levels deep (e.g., project/src/module/file)
        ...
        if !focus_area.is_some() {  // Only do this for whole-codebase analysis
            result.push_str("\n### Central Components\n\n");
            ...
            if let Ok(all_rels) = self.knowledge_graph.get_all_relationships() {
                for rel in all_rels {
                    *entity_relationship_counts.entry(rel.source_id.clone()).or_insert(0) += 1;
                    *entity_relationship_counts.entry(rel.target_id.clone()).or_insert(0) += 1;
                }
                ...
                for (entity_id, count) in entity_counts.iter().take(10) {
                    let entity = self.knowledge_graph.get_entity(&crate::graph::entity::EntityId::new(&entity_id.to_string()));
        ...
        if module_structure.contains_key("src") {
            if let Some(src_modules) = module_structure.get("src") {
                // Common layer names
                for layer in ["api", "controllers", "models", "services", "repositories", "utils", "core", "data", "ui", "views"].iter() {
        ...
        let entry_points = self.knowledge_graph.search("main entry").unwrap_or_default();

...
impl<'a> ResultFormatter<'a> {
    ...
    pub fn format(&self, entities: Vec<&dyn Entity>) -> Result<String> {
        match self.format {
            ...
            OutputFormat::Tree => self.format_tree(entities),
    ...
    fn format_tree(&self, entities: Vec<&dyn Entity>) -> Result<String> {
        if entities.is_empty() {
            return Ok("No entities found".to_string());
        }
        ...
        for entity in entities {
            ...
            let outgoing = self.kg.get_outgoing_relationships(entity.id());

            for (i, rel) in outgoing.iter().enumerate() {
                let is_last = i == outgoing.len() - 1;
                let prefix = if is_last { "└─ " } else { "├─ " };

                if let Some(target) = self.kg.get_entity(&rel.target_id) {
                    result.push_str(&format!(
                        "  {}{} ({:?}) <- {:?}\n",
                        prefix,
                        target.name(),
